...
-- あざみ野 vs vinculo: 0-4 → 0-4 (確認: 本サイトも0-4)
-- → 変更なし

-- あざみ野 vs 黒滝: 4-3 → 4-3 (確認: 本サイトも4-3)
-- → 変更なし
...
-- vinculo vs 黒滝: 2-0 → 2-0 (確認: 本サイトも2-0)
-- → 変更なし

-- vinculo vs TDFC: 7-1 → 7-1 (確認: 本サイトも7-1)
-- → 変更なし
...
-- 横浜 vs あざみ野K: 12-2 → 13-2 (※本サイトを再確認)
UPDATE matches
...
-- 黒滝 vs 東海岸: なし → 追加が必要（DBにはないが本サイトにはある？画像で確認）
-- 画像を見ると黒滝の東海岸欄は空白（-）なので追加不要
...
-- TDFC vs 東海岸: なし → 追加不要（画像でも空白）

-- PALAVRA vs ESFORCO: 1-3 → 0-8 (DBに2つある?)
-- DBには 1-3/0-8 と2つあるが、本サイトは1-3だけ。0-8を削除
DELETE FROM matches
...
-- あざみ野K vs ESFORCO: 0-8 → 0-8 (確認OK)
-- → 変更なし

-- あざみ野K vs 大豆戸: 0-8 → 0-8 (確認OK)
-- → 変更なし

-- あざみ野K vs あざみ野: 0-9 → 0-9 (確認OK)
-- → 変更なし

-- あざみ野K vs vinculo: 0-4 → 0-4 (確認OK)
-- → 変更なし
...
-- あざみ野K vs 黒滝: - → 本サイトでは空欄、DBにもないのでOK

-- あざみ野K vs TDFC: 0-4 → 0-4 (確認OK)
-- → 変更なし

-- あざみ野K vs 東海岸: - → 本サイトでも空欄、DBにもないのでOK
...
INSERT INTO matches (home_team_id, away_team_id, home_score, away_score, match_date, status, season_id)
SELECT '7b4c8e6b-2333-4414-8262-a7a05e8193c9', '45e82eee-9428-4b40-8568-bc4af17c48b1', 5, 0, '2024-06-22', 'finished', 'd0efd9f2-bca5-47dd-a70f-7b60f814a1dc'
WHERE NOT EXISTS (
...
-- 大豆戸 vs PALAVRA: 1-0 (本サイトでは1試合のみ、DBには6-1/3-0がある)
-- DBから6-1と3-0を削除して1-0を追加
DELETE FROM matches
WHERE home_team_id = '7b4c8e6b-2333-4414-8262-a7a05e8193c9'
AND away_team_id = '7d2c5a9d-3c44-4210-9599-b98dfa8bd626'
AND season_id = 'd0efd9f2-bca5-47dd-a70f-7b60f814a1dc';
...
INSERT INTO matches (home_team_id, away_team_id, home_score, away_score, match_date, status, season_id)
VALUES ('7b4c8e6b-2333-4414-8262-a7a05e8193c9', '7d2c5a9d-3c44-4210-9599-b98dfa8bd626', 1, 0, '2024-10-01', 'finished', 'd0efd9f2-bca5-47dd-a70f-7b60f814a1dc');

-- あざみ野 vs TDFC: なし → 本サイトも空欄なのでOK

-- vinculo vs 東海岸: 9-1/10-0 → 7-1 (12/20) のみ。10-0を削除
DELETE FROM matches
WHERE home_team_id = 'b57cd2ec-38ca-4765-91ce-9f1c920b5d48'
AND away_team_id = 'f6a92320-dad2-48a9-95e0-046e7004fd8b'
...
-- 黒滝 vs あざみ野K: なし → 4-0 (11/16)
INSERT INTO matches (home_team_id, away_team_id, home_score, away_score, match_date, status, season_id)
SELECT '90e69fe7-38b9-4bf7-afaa-e729ef95decc', '2e0cb14e-5a4f-440d-a341-3c9e6112355d', 4, 0, '2024-11-16', 'finished', 'd0efd9f2-bca5-47dd-a70f-7b60f814a1dc'
WHERE NOT EXISTS (
  SELECT 1 FROM matches
  WHERE home_team_id = '90e69fe7-38b9-4bf7-afaa-e729ef95decc'
  AND away_team_id = '2e0cb14e-5a4f-440d-a341-3c9e6112355d'
...
-- ESFORCO vs 東海岸: 9-0/22-0 → 22-0のみ残す。9-0を削除
DELETE FROM matches
WHERE home_team_id = '8a8da5d6-ed98-48f3-8140-783908399fa0'
AND away_team_id = 'f6a92320-dad2-48a9-95e0-046e7004fd8b'
...
-- WHERE m.season_id = 'd0efd9f2-bca5-47dd-a70f-7b60f814a1dc'
