...
export default function TeamStatsPage() {
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-gray-900">戦績</h2>

      {/* 成績サマリー */}
      {standing && (
        <div className="bg-white rounded-lg shadow-sm p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">今シーズンの成績</h3>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            <div className="text-center p-4 bg-blue-50 rounded-lg">
              <p className="text-sm text-gray-600">順位</p>
              <p className="text-3xl font-bold text-blue-600">{standing.rank || '-'}</p>
            </div>
            <div className="text-center p-4 bg-gray-50 rounded-lg">
              <p className="text-sm text-gray-600">試合数</p>
              <p className="text-2xl font-bold text-gray-900">{standing.matches_played}</p>
            </div>
            ...
            <div className="text-center p-4 bg-gray-50 rounded-lg">
              <p className="text-sm text-gray-600">得失点差</p>
              <p className="text-2xl font-bold text-gray-900">
                {standing.goal_difference > 0 ? `+${standing.goal_difference}` : standing.goal_difference}
              </p>
              <p className="text-xs text-gray-500">{standing.goals_for} - {standing.goals_against}</p>
            </div>
            <div className="text-center p-4 bg-yellow-50 rounded-lg">
              <p className="text-sm text-gray-600">勝点</p>
              <p className="text-3xl font-bold text-yellow-600">{standing.points}</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-gray-900">今後の試合</h3>
            <Link href={`/team-portal/${teamId}/schedule`} className="text-sm text-blue-600 hover:underline">
              全て見る
            </Link>
          </div>
          {upcomingMatches.length === 0 ? (
            <p className="text-gray-500 text-center py-8">予定されている試合はありません</p>
          ) : (
              ...
              {upcomingMatches.map((match) => {
                ...
                return (
                  <li key={match.id} className="flex items-center gap-4 p-3 bg-gray-50 rounded-lg">
                    ...
                    <span className="text-xs text-gray-500 truncate max-w-[100px]">{match.venue}</span>
                  </li>
                );
              })}
            </ul>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-gray-900">最近の試合</h3>
